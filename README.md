...
- [AnyAPI](https://getanyapi.com) - Unified marketplace for scraping and data APIs: reach hundreds of third-party data APIs through one `POST /v1/run/{sku}` endpoint (218 SKUs across social, e-commerce, and search), with schemas normalized and automatic failover across providers. Agents pay inline per call with no account via x402 (Base mainnet, USDC) or MPP (Tempo); humans pay per request in USD with no subscriptions. Listed in the Coinbase CDP x402 Bazaar. ([OpenAPI](https://api.getanyapi.com/openapi.json)) ([Docs](https://getanyapi.com/docs))
