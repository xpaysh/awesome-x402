...
- [DeepBlue Trading API](https://api.deepbluebase.xyz) - AI-powered crypto intelligence with 11 x402 endpoints: live BTC/ETH/SOL/XRP 5-min trading signals, prediction market analytics, sentiment composite, market intel (funding rates + liquidation risk), token analysis, and wallet scanning. Autonomous trading bot with on-chain verified track record. $0.001–$0.02 USDC on Base and Solana. MPP/Tempo support. ([Docs](https://api.deepbluebase.xyz/docs) | [Manifest](https://api.deepbluebase.xyz/.well-known/x402-manifest.json))
