...
- [Formo Profiles API](https://docs.formo.so/api/x402) - Wallet and user intelligence API for AI agents: turn any anonymous wallet address to a rich user profile. View net worth, chains, tokens, labels, and identity for any EVM address, with pay-per-request access via x402 USDC. Built for agents that need onchain identity and wallet-behavior context for personalization and lead enrichment.
