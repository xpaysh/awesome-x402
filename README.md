...
### Data & Social APIs
- [Proxygate](https://proxygate.ai) - Curated marketplace of real-world data APIs for AI agents. Connect once (MCP, SDK, CLI, or REST API) and call any listing (market data, crypto, weather, geocoding, dev tools, and more) from one prepaid USDC balance on Solana, topped up over x402. Seller keys are injected server-side, with a signed receipt per call. ([MCP](https://gateway.proxygate.ai/mcp) | [MCP Registry](https://registry.modelcontextprotocol.io/v0.1/servers?search=ai.proxygate))
