...
- [Casa](https://casa-agents.fly.dev) - Dropbox for AI agents — end-to-end encrypted, permanent file storage, paid per-call in USDC via x402. Upload any file, pay $0.01 USDC on Base, get back a permanent gateway URL. Stored forever on Arweave by default (pay-once endowment, ~200+ years; Swarm for large files); the client encrypts before upload so Casa is zero-knowledge (only your wallet can decrypt). Reads are free. No API keys, no signup. ([llms.txt](https://casa-agents.fly.dev/llms.txt) | [OpenAPI](https://casa-agents.fly.dev/openapi.json) | [.well-known/x402](https://casa-agents.fly.dev/.well-known/x402.json))
