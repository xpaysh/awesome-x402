...
- [Vibes-Coded](https://vibes-coded.com) - Pay-per-call reliability layer for AI agents: spend-caps that prevent orphaned leases at shutdown, idempotency guards for concurrent-replica settlement safety, contract-drift gates that catch schema drift before it propagates, and signed action receipts for post-revocation audit. 60 x402 endpoints on Base and Solana USDC. ([Discovery manifest](https://vibes-coded.com/.well-known/x402.json)) ([for-agents](https://vibes-coded.com/for-agents))
