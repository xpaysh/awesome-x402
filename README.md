...
### Data & Social APIs
- [Hermes Multifamily Deals Feed](https://hermes-storefront.onrender.com) - Deal-level US multifamily real-estate records for AI agents: 490+ active listings across 14 metros in 8 states with price, units, advertised cap rate, computed DSCR under declared loan assumptions, and HUD-SAFMR-underwritten NOI/cap that flags inflated broker pro-formas. Three-path Hot/Warm/Pass scoring with machine-readable rationales; typed schema with no silent nulls (every absent value carries a reason). Two tiers: full feed $0.05/call, Hot-list-only $0.02/call — USDC on Base via CDP facilitator, Bazaar discovery extension, refreshed every 2 days. No API keys, no signup. ([Discovery](https://hermes-storefront.onrender.com/.well-known/x402.json)) ([OpenAPI](https://hermes-storefront.onrender.com/openapi.json)) ([Free meta](https://hermes-storefront.onrender.com/api/meta)) ([Live status](https://hermes-storefront.onrender.com/office))
