...
### Data & Social APIs
- [Predge](https://data.predge.io) — Outcome-verified prediction-market whale intelligence for AI agents: Polymarket whale trades ≥$10k, wallet conviction scoring over resolved markets, and smart-money consensus, plus two flagship attestations — `predge_attest` (signed resolved Polymarket outcome) and `predge_sports_attest` (ed25519-signed settled game result) — that any agent can verify offline. Pay-per-call in USDC on Base and Solana mainnet via x402 (PayAI facilitator), no accounts or API keys ($0.005–$0.03/call). Also an MCP server (`npx -y @predge/whale-data-mcp`, 17 tools) in the official MCP Registry as `io.github.predgeAI/whale-data-mcp`. ([Discovery](https://data.predge.io/.well-known/x402)) ([npm](https://www.npmjs.com/package/@predge/whale-data-mcp)) ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=predge))
