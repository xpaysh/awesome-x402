...
- [TO1 Intelligence Fleet](https://to1.deadbeatradar.com) - 90 pay-per-call MCP data servers (297 tools: markets, security guards, geo, science, health of other MCP tools) behind one gateway endpoint. x402 USDC on Base from $0.01/call, free trial via `X-Trial` header, spec-compliant JSON-RPC 402s with `accepts[]` in `error.data`. ([Gateway MCP](https://to1.deadbeatradar.com/mcp/fleet) | [Catalog](https://to1.deadbeatradar.com/fleet.json) | [MCP Registry](https://registry.modelcontextprotocol.io/v0.1/servers?search=to1-fleet))
