...
#- [MarketNow](https://marketnow.site) — MCP marketplace with 8,845 security-audited skills. 10-layer Sentinel audit (L1.5→L1.9 + L2 sandbox + L3 continuous monitoring), Agent Trust Cards (ATC, Ed25519 signed), USDC payments on Base via x402-compatible mandates. All skills free to install. 8 MCP tools: search, recommend, submit, verify_trust, install. `npx -y marketnow-mcp` (v1.5.1). ([GitHub](https://github.com/edgarfloresguerra2011-a11y/marketnow)) | ([npm](https://www.npmjs.com/package/marketnow-mcp)) | ([ATC](https://marketnow.site/atc)) | ([Security API](https://marketnow.site/api/security))
