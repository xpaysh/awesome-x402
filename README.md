...
- [Agent Café](https://402.coffee) — The trust layer for agent payments on Base. Three services on one live x402 endpoint: **certify** a paying agent (it pays real USDC and earns a public certificate + README badge proving it pays correctly, refuses over-priced scams, and checks who it's paying), **score** any wallet's payment risk 0–100 before you transact (`POST /score` $0.10; free `GET /verify`), and **escrow** your own x402 sales with automated on-chain arbitration (non-custodial, 1% fee, buyer refunds enforced on-chain). Free machine-readable menu at [/inspect](https://api.402.coffee/inspect). [Docs](https://api.402.coffee/docs) | [Examples](https://github.com/englishdoggy/agentcafe-examples)
