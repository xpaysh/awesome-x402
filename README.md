...
- [Solana Data API](https://really-operates-beans-memories.trycloudflare.com) - Pay-per-request Solana blockchain data API for AI agents. 6 endpoints: wallet balance, token price, transaction lookup, recent transactions, top tokens, and custom RPC proxy. USDC micropayments on Base Mainnet via x402. $0.001-$0.005 per call, no API keys, no signup. ([Discovery](https://really-operates-beans-memories.trycloudflare.com/.well-known/x402/services))
