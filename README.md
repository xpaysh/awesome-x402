...
- [x402tools](https://x402tools.xyz) - 9 pay-per-call utility APIs for AI agents on Base: QR codes ($0.01), website screenshots ($0.05), DNS lookup ($0.02), HTML/PDF document parsing ($0.01), prompt-injection screening ($0.03), email validation ($0.03), HTML-to-PDF rendering ($0.05), image OCR ($0.05), and sales-prospect enrichment ($0.50). USDC micropayments, no API keys or signup; free `/preview` + `/health` on every service. Indexed in the Coinbase Bazaar via the CDP facilitator. MCP: `npx -y @x402-tools/mcp` (10 tools; also hosted at mcp.x402tools.xyz). ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=x402tools) | [Discovery](https://x402tools.xyz/.well-known/x402-services.json) | [OpenAPI](https://x402tools.xyz/openapi.json) | [GitHub](https://github.com/acrylicfiddle/x402-services))
