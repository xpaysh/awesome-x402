...
- [Brazil Data API](https://brasil-dados-api.onrender.com) - Official data about Brazil for AI agents: Central Bank economic indicators (SELIC, CDI, IPCA, PTAX, Focus market forecasts), KYB company dossiers (legal name, registration status, partners/QSA, CNAE) with automatic CEIS sanctions screening, CPF/CNPJ tax ID and CEP postal code lookups, and universal verification (email, phone, IBAN, card) with timestamped SHA-256 receipts. Batch endpoints for bulk workflows (up to 25 items per call). $0.001-$0.01 USDC per call, dual-rail payments on Base and Solana mainnet (gas sponsored by the facilitator — no ETH/SOL needed), CDP-facilitated, Bazaar-listed. ([llms.txt](https://brasil-dados-api.onrender.com/llms.txt)) ([OpenAPI](https://brasil-dados-api.onrender.com/openapi.json)) ([GitHub](https://github.com/pconopca/brasil-dados-api))
