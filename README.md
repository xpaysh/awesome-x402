...
- [elsas.it](https://elsas.it) - Daily security intelligence reports for AI agent stacks, sold per-request for $0.10 USDC via x402 with a self-hosted facilitator. Free sample report available via MCP without payment.
...
- [Elsas](https://elsas.it) - Daily, Ed25519-signed security intelligence for AI-agent stacks. Curated CVEs, supply-chain incidents, and advisories (GHSA / CISA-KEV / OSV / NVD + national-CERT feeds) assessed for impact on MCP servers, LLM proxies, and orchestration tools, then independently cross-validated and signed. 1 paid tool `get_today` ($0.10 USDC on Base) plus free `get_sample_report`, `list_available`, `check_affected`, and `get_since`. No accounts — x402 pay-per-report; every report is offline-verifiable without trusting the server. ([MCP](https://elsas.it/mcp)) | ([Sample](https://elsas.it/sample)) | ([Verify kit](https://github.com/romans-repos/elsas-verify))
