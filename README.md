...
### API Examples
- [Autonomagic x402 API](https://api.autonomagic.org) - 18 paid endpoints for AI agents on Base mainnet: 3 LLM (echo $0.01, summarize $0.05, generate $0.10) + 15 utility endpoints (web scrape, DNS lookup, crypto price feeds, multi-URL fetch, HTTP health checks, text hash, UUID, encode, JSON validate, diff, regex test, word-count, random data, signed timestamps). $0.001-$0.10 USDC per call. Self-facilitated EIP-3009 settlement on Base — no third-party facilitator. 50% of all revenue auto-splits to creator wallet via on-chain heartbeat. OpenAPI 3.0 + A2A agent-card + x402 manifest discovery. [OpenAPI](https://api.autonomagic.org/openapi.json) | [Agent Card](https://api.autonomagic.org/.well-known/agent-card.json) | [x402 Manifest](https://api.autonomagic.org/.well-known/x402.json)
