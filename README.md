...
- [x402 Whois FR](https://x402-whois.suretat.com) — Domain WHOIS with GDPR-aware output for .fr and generic TLDs. Returns registrar, creation/expiration/update dates, nameservers, domain status codes, and days-until-expiry. RGPD-compliant: owner details masked for individual registrations. `POST /whois` with `{"domain": "example.fr"}`. 0.001 USDC/call on Base. ([Docs](https://x402-whois.suretat.com/docs) | [/.well-known/x402.json](https://x402-whois.suretat.com/.well-known/x402.json))
- [x402 Email Validator](https://x402-email.suretat.com) — Deep email validation: RFC 5321 syntax check, live DNS MX lookup, disposable domain detection (~200 known providers), and deliverability score estimate. `POST /validate` returns `valid`, `mx_found`, `is_disposable`, `deliverability_score` (0–100). 0.001 USDC/call on Base. ([Docs](https://x402-email.suretat.com/docs) | [/.well-known/x402.json](https://x402-email.suretat.com/.well-known/x402.json))
