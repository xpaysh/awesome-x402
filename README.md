...
- **[Verity (VerityLayer)](https://api.veritylayer.dev/.well-known/x402.json)** — Fail-closed verify-before-you-act trust gate for AI agents. Returns a verdict (supported/unsupported/uncertain) plus a calibrated 0–1 confidence and evidence, and abstains rather than guess. Keyless, pay-per-call in USDC on Base (eip155:8453). Three tiers: `/verify/quick` $0.02 (Haiku pre-filter) · `/verify` $0.25 (Sonnet, grounded with live citations) · `/verify/pro` $0.35 (Opus). Also ships an MCP server (`io.github.meloliva14/verity-mcp`). ([Manifest](https://api.veritylayer.dev/.well-known/x402.json) | [Site](https://veritylayer.dev))
