...
- [tzapi](https://tzapi.vercel.app) - Time zone, datetime conversion, and business-hours lookups for AI scheduling/booking agents. `/now`, `/convert`, `/business-hours` accept city name or lat/lon, return IANA timezone, UTC offset, DST status, weekday, and country-holiday-aware open/closed status. Built on `timezonefinder` + IANA tz database + `python-holidays`. $0.001–$0.002 USDC per call on Base. No API keys, no signup. _Try: `curl https://tzapi.vercel.app/now?city=Tokyo`_
- [md-api](https://md-api.vercel.app) - URL → clean article markdown for RAG and AI research agents. SSRF-guarded (private/internal IPs refused after DNS resolution, 5 MB response cap, 10s timeout, max 5 redirects). Optional `include_images` and `metadata` (title, author, date, sitename) flags. Built on `trafilatura` + `markdownify`. $0.002 USDC per call on Base. _Try: `curl 'https://md-api.vercel.app/md?url=https://example.com'`_
- [holidays-api](https://holidays-api-kappa.vercel.app) - Public holiday and business-day calendar for 100+ countries with sub-national support. `/is-holiday` (single date), `/next-business-day` (skip weekends + holidays), `/year` (all holidays for a year), free `/countries`. Built on `python-holidays`. For invoicing, SLA, and notification-timing agents. $0.001–$0.005 USDC per call on Base. _Try: `curl 'https://holidays-api-kappa.vercel.app/is-holiday?country=US&date=2026-12-25'`_
- [langdetect-api](https://langdetect-api.vercel.app) - Language detection across 75 languages using Lingua's statistical models. Returns ISO 639-1 code, language name, confidence, and ranked candidates. `/detect` (GET or POST), `/detect-multi` for mixed-language span detection. $0.001 USDC per call on Base. _Try: `curl 'https://langdetect-api.vercel.app/detect?text=Hello+world'`_
- [calparse-api](https://calparse-api.vercel.app) - Natural-language datetime parser for AI scheduling/calendar agents. Resolves expressions like `next Tuesday at 3pm`, `in 5 minutes`, `mañana` to ISO 8601 in any IANA timezone. Two-pass: `parsedatetime` (English relative idioms) → `dateparser` (multilingual fallback). `/parse` (GET or POST). $0.001 USDC per call on Base or Solana mainnet. Sister to tzapi. _Try: `curl 'https://calparse-api.vercel.app/parse?text=next+Tuesday+at+3pm&tz=Australia/Sydney'`_
- [x402rugproof](https://rugproofx402.vercel.app) - Solana bundle / sniper / funder-cluster scoring for AI trader agents. Pre-graduation pump.fun mints: returns `verdict`, `flags`, per-wallet drill-down, `top_cluster_pct`, `n_established`, `n_fresh`. Helius-only derived (no Birdeye/GMGN/Nansen resale — clean ToS). `GET /score/{mint}`. $0.05 USDC per call on Solana mainnet via PayAI facilitator (gasless for buyers). MIT-licensed. _Try: `curl https://rugproofx402.vercel.app/score/<mint>`_
- [x402rent](https://solrent402.vercel.app) - Solana empty-ATA rent recovery for AI portfolio agents. Returns unsigned transaction closing empty SPL Token accounts to reclaim ~0.002 SOL each (~$0.40). No key custody — service never signs. Bundles up to 25 closes per tx. `GET /close-empty-atas?wallet=<pubkey>`. $0.05 USDC per call on Solana mainnet via PayAI (gasless). MIT-licensed. _Try: `curl 'https://solrent402.vercel.app/close-empty-atas?wallet=<pubkey>'`_
- [x402sniper](https://x402sniper.vercel.app) - Post-graduation Solana sniping decision support for AI trader bots. Combines bundle check + launchpad detection → `SNIPE` / `WATCH` / `SKIP` verdict with score, reasons, and full drill-down. Detects self-seeded pools (dev seeded LP directly). Signal only — no execution. `GET /score/{mint}`. $0.05 USDC per call on Solana mainnet via PayAI (gasless). MIT-licensed. _Try: `curl https://x402sniper.vercel.app/score/<mint>`_
