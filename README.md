...
- [Formo Profiles API](https://docs.formo.so) - Wallet intelligence API for AI agents: net worth, chains, tokens, labels, and identity for any EVM address, with pay-per-request access via x402 USDC. Built for agents that need onchain identity and wallet-behavior context before acting.
...
- [Formo Profiles API](https://docs.formo.so) - x402-enabled wallet profile and onchain attribution API for AI agents. Query wallet intelligence, DeFi/crypto user profiles, and cross-chain attribution data pay-per-call on Base.
