...
- [Kelam](https://pay.kelam.sh) - Real phone calls for AI agents, live on Base. Give a phone number and a task ("call this restaurant and book a table for 4 at 7pm") and a voice AI places the call and returns the outcome + full transcript. Also provision your own inbound/outbound voice agents, fund inbound minutes, and drive the agent builder in plain English — all pay-per-use in USDC via x402 (Coinbase CDP facilitator), no account or API key. Calls are metered per-minute (connect fee + $0.20/min, capped; unconnected calls free); create_agent $2, builder $0.50/msg. ([OpenAPI](https://pay.kelam.sh/openapi.json) | [llms.txt](https://pay.kelam.sh/llms.txt))
