...
- [Dispatch x402](https://dispatch-x402.fly.dev) — x402 Aggregator Meta-API. One paid endpoint routes an AI agent's natural-language intent to the right paid service across the Royal Agentic fleet (skill discovery, email scoring, consumer-law lookup, dispute packs, vault scaffolding, ticker analysis, image gen, NFT signals). Free `/classify` previews the chosen service + calibrated confidence — no charge for ambiguous routes; paid `/dispatch` returns the authoritative route, params extracted from the intent, and ready-to-call instructions. Routing itself is the priced primitive. Claude Haiku classifier with keyword fallback. $0.50 USDC per dispatch on Base mainnet. [Discovery](https://dispatch-x402.fly.dev/.well-known/x402.json) | [Source](https://github.com/bshelby88/dispatch-x402)
- [TradingAgents x402](https://tradingagents-x402.fly.dev) — Multi-agent LLM ticker consensus. Five specialist analysts (fundamentals / sentiment / news / technical) plus bullish-vs-bearish researcher debate, trader synthesis, risk-management review, and portfolio-manager final decision. Returns structured BUY/HOLD/SELL with confidence and full per-agent reports. Powered by [TauricResearch/TradingAgents](https://github.com/TauricResearch/TradingAgents) (arXiv:2412.20138). $0.05 USDC per call on Base mainnet. [Source](https://github.com/bshelby88/tradingagents-x402)
...
- [Nano Banana x402](https://nanobanana-x402.fly.dev) — Paid Gemini image generation. Wraps `gemini-3.1-flash-image-preview` with x402 micropayments. Generate from prompt OR edit an existing image. Returns base64 PNG. Supports 10 aspect ratios. $0.01 USDC per call on Base mainnet. [Source](https://github.com/bshelby88/nanobanana-x402)
