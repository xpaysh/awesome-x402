...
- [token-risk](https://token-risk.com) - Deterministic on-chain **structural** risk reports for ERC-20 tokens and wallet addresses on Base. Reports structure, not reputation: centralized-control powers (mint/pause/blacklist) flagged regardless of issuer. Score 0-100. $0.01 USDC per call, keyless x402-native. Free real sample reports before paying: [/samples/index.json](https://token-risk.com/samples/index.json). ([OpenAPI](https://token-risk.com/openapi.json)) ([AgentCash skill](https://github.com/Merit-Systems/agentcash-skills/pull/23))
