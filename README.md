...
- [FrameFetch](https://framefetch.net) - Any social-video URL → metadata, transcript (captions or Whisper), engagement insights, and parametrically-sampled frames (every Nth / 1-per-second / time-range, any width), for AI agents. Six platforms through one JSON schema — YouTube, Shorts, TikTok, Instagram Reels, Pinterest, and Reddit, including Reddit-video transcripts. Pay per call in USDC via x402 on Base with no account (metadata sub-cent, transcript per minute, frames per frame); humans can use a free tier or Stripe. REST + MCP, result caching, and a no-signup demo. ([MCP Server](https://framefetch.net/mcp)) ([OpenAPI](https://framefetch.net/openapi.json)) ([Discovery](https://framefetch.net/.well-known/x402.json)) ([Docs](https://framefetch.net/docs)) ([GitHub](https://github.com/MarvinRey7879/framefetch))
