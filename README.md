...
- [DoNotAct](https://donotact.com) - Fail-closed risk gateway for prediction-market and token agents before capital moves. x402 per-call endpoints return machine-readable `READY`, `CAUTION`, `DO_NOT_ACT`, or `INSUFFICIENT_EVIDENCE` verdicts for UMA dispute lifecycle, resolution ambiguity, requested size vs public book capacity, and token T1-T8 risk checks. Signed receipts, MCP endpoint, OpenAPI, and llms.txt. $0.01-$0.02 USDC per paid check on Polygon. ([x402](https://donotact.com/docs/x402) | [MCP](https://donotact.com/mcp) | [OpenAPI](https://donotact.com/openapi.json))
