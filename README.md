...
- [AgentFilings](https://api.agentfilings.com) - Structured US SEC/EDGAR filing data for AI agents: filings index, XBRL-derived earnings summaries, and Form 4 insider transactions as clean JSON. Covers 498 S&P 500 companies, re-ingested twice daily from public-domain SEC EDGAR, with every earnings period and insider transaction carrying its source accession number. $0.01-$0.02 USDC per call via x402 on Base mainnet, no accounts or API keys; a free /probe endpoint reports coverage, freshness and per-endpoint pricing so an agent can check before paying. ([OpenAPI](https://api.agentfilings.com/openapi.json) | [llms.txt](https://api.agentfilings.com/llms.txt) | [MCP](https://api.agentfilings.com/mcp))
