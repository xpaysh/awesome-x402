...
- [AgentFlow](https://agentflow.watch) - Wash/sybil screening + the Real-Agent Index for Base agent wallets. Ask any wallet "real autonomous agent or fake-volume sybil?" and get a calibrated score + reasons. Pay-per-call x402 USDC on Base, $0.01 (bulk/subscription/signed-attestation tiers too). Free MCP server (Streamable HTTP, 5 read tools) + the honest real-vs-fake agent benchmark. No API keys, no signup. ([MCP](https://agentflow.watch/mcp) | [Benchmark](https://agentflow.watch/index.json))
- 
