...
- [Grey Ridge Signals — x402 Data & Security](https://x402-data-api.sigrunner.workers.dev) - Agent-native pay-per-call on Base USDC. `GET /pm/markets` ($0.005) — live Polymarket prediction-market data (question, outcomes, live prices, volume, liquidity, end date; keyword filter). Plus MCP security audits — `scan_mcp_server` ($0.05) / `GET /scan/mcp` ($0.10) scan any MCP server for tool-poisoning / prompt-injection / data-exfiltration (OWASP LLM01/LLM08), and tech-stack→CVE/EPSS/CISA-KEV risk enrichment. No account, no API key. x402 v2 with CDP Bazaar discovery. ([Discovery](https://x402-data-api.sigrunner.workers.dev/.well-known/x402) | [MCP](https://x402-data-api.sigrunner.workers.dev/mcp) | [GitHub](https://github.com/rezearcher/mcp-security-scanner))
