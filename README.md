...
- [SwapTitan](https://swaptitan.net) - No-KYC cross-chain crypto swap API for AI agents — 1288+ assets with automatic best-rate routing across multiple providers. **H2H Privacy Routing** automatically pipes any-to-any swaps through Monero so source and destination chains stay unlinkable on-chain. 18 MCP tools: Solana DEX swaps (Jupiter), EVM swaps on 7 chains (KyberSwap), cross-chain `swap_quote`/`swap_create`/`swap_status`, `get_prices`, `rug_check`, Hyperliquid perps and more. $0.02 USDC on Base per MCP tool call (100 free/day per IP), $0.05 per `swap_create`, or €49.99/mo Agent Tier via `X-License-Key` header. On-chain x402 verification. ([MCP Server](https://swaptitan.net/mcp)) ([Pricing](https://swaptitan.net/pricing)) ([GitHub](https://github.com/polsolbridge/solana-swap-api)) ([Agent Card](https://swaptitan.net/.well-known/agent.json))
