...
- [x402 Adresses FR](https://x402-adresses.suretat.com) — French postal address validation & geocoding via Base Adresse Nationale (data.gouv.fr). Normalizes addresses, returns INSEE code, GPS coordinates. 0.001 USDC/call on Base. ([/.well-known/x402.json](https://x402-adresses.suretat.com/.well-known/x402.json))
- [x402 SIRET/SIREN FR](https://x402-siret.suretat.com) — French company lookup via INSEE SIRENE (data.gouv.fr). Returns legal name, address, NAF code, directors (up to 5), admin status. Two endpoints: `POST /siret` and `POST /siren`. 0.003 USDC/call on Base. ([/.well-known/x402.json](https://x402-siret.suretat.com/.well-known/x402.json))
- [x402 IBAN Validator](https://x402-iban.suretat.com) — IBAN checksum validation (50+ countries, ISO 13616) with RIB decomposition for FR/GB/BE. Pure computation, no external API. 0.0005 USDC/call on Base. ([/.well-known/x402.json](https://x402-iban.suretat.com/.well-known/x402.json))
- [x402 Geocoder FR](https://x402-geocoder.suretat.com) — French address geocoding: `POST /geocode` (address→GPS), `POST /reverse` (GPS→address), `POST /commune` (lookup by name/INSEE/postal code). Uses official BAN + geo.api.gouv.fr APIs. 0.001 USDC/call on Base. ([/.well-known/x402.json](https://x402-geocoder.suretat.com/.well-known/x402.json))
- [x402 Scraper API](https://x402-scraper.suretat.com) — Web scraping API: `POST /scrape` extracts title, description, Open Graph, JSON-LD, links and main text from any URL. `POST /extract` for named CSS selectors. 0.002 USDC/call on Base. ([/.well-known/x402.json](https://x402-scraper.suretat.com/.well-known/x402.json))
