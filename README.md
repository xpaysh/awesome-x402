...
- [x402engine](https://x402engine.app) — Pay-per-call API gateway with 74 x402-payable endpoints across 10 categories (44 LLMs, image/video gen, crypto data, web search, code execution, TTS, travel, IPFS). Multi-chain: USDC on Base, USDm on MegaETH, USDC on Solana. MCP server, agent discovery via `/.well-known/agent.json` and `/.well-known/agent-manifest.v1.json`. ([GitHub](https://github.com/agentc22/x402-engine)) | ([MCP](https://www.npmjs.com/package/x402engine-mcp))
