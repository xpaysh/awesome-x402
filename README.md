...
- [Nervous System x402 API](https://api.100levelup.com/openapi.json) - External governance layer for agent systems: pay-per-call audit-chain verification ($0.005) and deterministic governance lint for MCP server configs ($0.05) covering plaintext secrets, unpinned packages, auto-install flags, filesystem scope, and transport security. USDC on Base via x402 v2 with v1 header compat. Self-hosted facilitator, no signup, no API keys. ([llms.txt](https://api.100levelup.com/llms.txt)) ([Site](https://levelsofself.com))
