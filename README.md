...
- [AgentRank](https://agentrank.info) - Counterparty trust ranked by **real on-chain settlement**, not reputation feedback or scam lists. PageRank on the x402 USDC payment graph (de-noised, sybil-tested): a payee is `verified` only if it has settled value traceable to reputable payers, so ghosts and sybils score ~0 because you cannot fake real money. ~78% of x402 recipients are 1-payer ghosts; AgentRank scores the real ones 0-1000. Pre-pay check at `GET /resolve/{wallet|domain}`, an MCP tool `check_agent_trust`, an embeddable badge, and a zero-dep verify-before-pay `fetch` wrapper. ([API](https://api.agentrank.info)) | ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=agentrank)) | ([Methodology](https://github.com/andysalvo/agentrank))
...
- [AgentRank](https://api.agentrank.info/mcp) - `check_agent_trust(wallet|domain)`: is this agent or x402 service real and settlement-backed before you pay it? Returns a verified flag, a settlement-grounded score (0-1000), and the real USDC settled. Ranked by PageRank on the x402 payment graph, not reputation feedback. Streamable HTTP. ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=agentrank)) | ([API](https://api.agentrank.info))
