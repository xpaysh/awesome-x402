...
- [Suede AI](https://app.suedeai.ai) — Rights-aware AI music and video generation with on-chain provenance on Base. Five paid endpoints: `/agent/generate` and `/create-music` ($0.20 USDC each, music), `/agent/video` ($1.50 USDC, short video), `GET /v1/rights/{assetHash}` ($0.005 USDC, Suede Registry attestation lookup), and `POST /v1/analyze` ($0.003 USDC, BPM/key/energy audio analysis). Returns `402 Payment Required` with `discoverable: true` per Bazaar convention. Discovery via [/.well-known/x402](https://app.suedeai.ai/.well-known/x402), [/.well-known/x402.json](https://app.suedeai.ai/.well-known/x402.json), and [/.well-known/agent-card.json](https://app.suedeai.ai/.well-known/agent-card.json). USDC on Base via Coinbase x402 facilitator. ACP commerce intent endpoint at `/agents/commerce`. ([ElizaOS plugin](https://www.npmjs.com/package/@aisuede/plugin-suede))
