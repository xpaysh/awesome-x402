...
- [PayAPI Market](https://payapi.market) - Marketplace for x402 APIs where the verified badge means the listing was paid for from the operator's own wallet and returned real product, not scored by heuristics. 28 live APIs from 15 independent providers, 26 currently carrying the badge. Coverage spans compliance and sanctions screening, company registries, government open data, crypto and DeFi market data, news and research, and agent tooling. $0.001-$1.77 USDC per request. MCP server for agent discovery. ([GitHub](https://github.com/chetparker/x402-marketplace))
