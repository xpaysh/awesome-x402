...
- [Agent Web Reader](https://x402.charliemorrison.dev) - Pay-per-call data tools for AI agents: crypto market signal (price/trend/momentum for a symbol), clean web-content reader (readable text + title/description/canonical + outbound links as JSON, the data an agent ingests each crawl/RAG step), and GitHub-repo SEO audit (0-100 discoverability score + concrete fixes). $0.005 USDC per call on Base, no API key, no signup. Listed on the official MCP Registry (`dev.charliemorrison/agent-web-reader`) + x402scan. ([MCP](https://x402.charliemorrison.dev/mcp) | [OpenAPI](https://x402.charliemorrison.dev/openapi.json) | [Discovery](https://x402.charliemorrison.dev/.well-known/x402) | [GitHub](https://github.com/charlie-morrison/x402-agent-data))
