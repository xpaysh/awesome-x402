...
- [patternfetch](https://patternfetch.com) - Token-compact market-state briefs for AI trading agents. One pay-per-call turns a crypto ticker + timeframe into compact candles + a SAX shape signature, detected patterns, support/resistance, regime, and interpreted indicators (RSI/EMA/ATR). Crypto-first, impersonal/non-directive. $0.005–$0.05 USDC on Base via x402 (Stripe for humans). ([Discovery](https://patternfetch.com/.well-known/x402.json) | [OpenAPI](https://patternfetch.com/openapi.json) | [MCP Server](https://patternfetch.com/mcp))
- [FrameFetch](https://framefetch.net) - Agent-first video-data API across 6 platforms (YouTube/Shorts, TikTok, Reddit, Instagram, Pinterest): metadata + insights, Whisper transcript, and parametric frames (fps or exact timestamps → S3). Typed errors, refund-on-fail, caching. Pay-per-call in USDC on Base via x402 (Stripe for humans). ([OpenAPI](https://framefetch.net/openapi.json) | [MCP Server](https://framefetch.net/mcp))
### Games & On-Chain Apps
