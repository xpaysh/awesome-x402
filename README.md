...
- [Deepnets](https://api.deepnets.ai) - Solana token intelligence for trading agents. 16 pay-per-call endpoints: token safety analysis, holder & funding-network graphs (catches scam bundles & wallet clusters), AI-generated social research, Streamflow vesting locks, and a curated watchlist + trending feed. $0.01 USDC per call ($0.15 for the full holder breakdown). x402 on Solana (USDC), no signup or API keys, OpenAPI spec, MCP server, A2A Agent Card. ([Discovery](https://api.deepnets.ai/.well-known/x402)) ([OpenAPI](https://api.deepnets.ai/openapi.json)) ([Agent Card](https://api.deepnets.ai/.well-known/agent-card.json)) ([MCP Config](https://api.deepnets.ai/.well-known/mcp.json)) ([MCP server on npm](https://www.npmjs.com/package/@deepnets/mcp-server)) ([Agents Guide](https://deepnets.ai/agents))
