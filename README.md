...
- [x402 Translate FR](https://x402-translate.suretat.com) — Professional-grade text translation via DeepL API. 30 languages, `POST /translate` with `{"text": "...", "source": "fr", "target": "en"}` (source defaults to auto-detect). Returns translated text, detected source language, and character count. `GET /languages` lists all supported codes. 0.001 USDC/call on Base (up to 5000 chars). ([Docs](https://x402-translate.suretat.com/docs) | [/.well-known/x402.json](https://x402-translate.suretat.com/.well-known/x402.json))
- [x402 IP Geolocation](https://x402-ipgeo.suretat.com) — IP address geolocation and network intelligence. `POST /lookup` with `{"ip": "8.8.8.8"}` returns country, region, city, ISP/ASN, GPS coordinates, timezone, and proxy/VPN/hosting detection flags. Supports IPv4 and IPv6. 0.001 USDC/call on Base. ([Docs](https://x402-ipgeo.suretat.com/docs) | [/.well-known/x402.json](https://x402-ipgeo.suretat.com/.well-known/x402.json))
