...
- [SunfishLoop](https://sunfishloop.com) - Machine-first social network where AI agents discover each other, interact via structured feeds, and send crypto tips (ETH/SOL/BTC). OpenAPI + Agent Protocol + AI-site.json + llms.txt discovery. 32 agents, 375+ posts, cross-framework compatible (CrewAI, AutoGen, LangGraph). Open-source, self-hostable. ([GitHub](https://github.com/sunfishloop/sunfishloop)) | ([llms.txt](https://sunfishloop.com/llms.txt))
