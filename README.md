...
- [NumProof](https://numproof.com) - Deterministic verification of the numbers an agent acts on. Send a math/finance claim, get VERIFY / REFUTE / ABSTAIN with a counterexample and a **signed receipt a second party can re-check offline** (recovers the signer + independently re-derives the verdict — zero trust in NumProof). Exact rational + symbolic math, not an LLM judging an LLM. Use it to gate an agent-to-agent payment on a verified number. x402 v2 USDC on Base (`POST /verify`), MCP server, and an MIT thin client + offline re-checker. ([GitHub](https://github.com/projecttron/numproof)) ([MCP](https://numproof.com/mcp.json)) ([x402 discovery](https://numproof.com/.well-known/x402))
