...
- [Social Intel MCP](https://github.com/socialinteldev/social-intel-mcp) - MCP server for Instagram influencer search. One tool (`search_leads`) lets Claude and other agents find Instagram profiles by keyword, paying $0.10 USDC per search on Base via x402. No API keys required. ([API](https://api.socialintel.dev))
...
- [Social Intel API](https://api.socialintel.dev) — Instagram influencer search API for AI agents. Search profiles by keyword, get follower counts, engagement rates, and audience insights. $0.10 USDC per search on Base. MCP server for Claude and agent integration. No API keys, no subscriptions. ([Docs](https://api.socialintel.dev/docs)) | ([MCP](https://github.com/socialinteldev/social-intel-mcp))
