...
- [SameDayDesk x402 Gateway](https://x402-url-extractor-production.up.railway.app) - Multi-endpoint pay-per-call gateway on Base (USDC, no auth, no keys): /enrich (domain -> company intelligence: identity, tech stack, social, contact, DNS, AI-readiness), /extract & /read (URL -> structured JSON / Markdown), /scan (pre-install GitHub repo security scan), /schemaforge (paste-ready JSON-LD). ([Discovery](https://x402-url-extractor-production.up.railway.app/.well-known/x402))
