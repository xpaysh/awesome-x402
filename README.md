...
- [x402 Email Validator](https://x402-email-validator.suretat.com) — RFC 5321 email validation with live DNS MX lookup and disposable address detection (~200 blocked domains). `POST /validate` returns syntax check, MX record status, deliverability score, and disposable flag. 0.001 USDC on Base. Self-hosted, no API key required. ([Docs](https://x402-email-validator.suretat.com/docs) | [.well-known](https://x402-email-validator.suretat.com/.well-known/x402.json))
- [x402 IP Geolocation](https://x402-ipgeo.suretat.com) — IP geolocation for IPv4 and IPv6 via ip-api.com. `POST /geolocate` returns country, region, city, ISP, latitude/longitude, timezone, and ASN. 0.001 USDC on Base. No API key, no signup. ([Docs](https://x402-ipgeo.suretat.com/docs) | [.well-known](https://x402-ipgeo.suretat.com/.well-known/x402.json))
- [x402 Translate FR](https://x402-translate-fr.suretat.com) — Professional translation via DeepL API covering 30 languages. `POST /translate` accepts up to 5000 characters, source language auto-detected. 0.001 USDC on Base. ([Docs](https://x402-translate-fr.suretat.com/docs) | [.well-known](https://x402-translate-fr.suretat.com/.well-known/x402.json))
- [x402 WHOIS FR](https://x402-whois-fr.suretat.com) — WHOIS lookup for .fr and generic TLD domains. `POST /lookup` returns registrant (RGPD-masked for individuals), creation/expiry/update dates, nameservers, registrar, and domain status (active, redemptionPeriod…). 0.001 USDC on Base. ([Docs](https://x402-whois-fr.suretat.com/docs) | [.well-known](https://x402-whois-fr.suretat.com/.well-known/x402.json))
...
- [x402 Color Converter](https://x402-color.suretat.com) — Color format conversion and manipulation. `POST /convert` converts any color (HEX, RGB, HSL, HSV, CMYK, named) to all formats simultaneously, with complementary/analogous/triadic harmonies. 0.0005 USDC on Base. ([Docs](https://x402-color.suretat.com/docs) | [.well-known](https://x402-color.suretat.com/.well-known/x402.json) | [GitHub](https://github.com/fredericsuretat/x402-suretat))
