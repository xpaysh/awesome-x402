...
### Data & Social APIs
- [Reputa](https://reputa.xyz) - On-chain wallet risk & security intelligence for Ethereum. Free `POST /api/crisis-check` returns a crisis-behaviour verdict (how a wallet acted through Terra, Celsius, FTX, Voyager, Multichain, Humanity) plus a security snapshot: unlimited token approvals, flagged-address contact (mixer/scam/OFAC), protocol usage, concentration. Paid `POST /api/approvals` deep approval scan via x402 (USDC on Polygon, self-hosted facilitator). No accounts. ([OpenAPI](https://reputa.xyz/openapi.json)) ([x402 descriptor](https://reputa.xyz/.well-known/x402.json)) ([npm MCP](https://www.npmjs.com/package/reputa-mcp))
