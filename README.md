...
- [NodeProxy](https://nodeproxy-production.up.railway.app) - URL→clean-Markdown parser for LLM ingestion: fetches any public URL, strips scripts/ads/nav, and returns token-efficient semantic Markdown. Two tools: `surface_markdown_parser` ($0.002) and `stealth_markdown_parser` ($0.05, residential-proxy + hardened Playwright for Cloudflare/Akamai pages). USDC on Base (also Polygon, Arbitrum, Solana). No API key, no signup. MCP-compatible; Python/LangChain + CrewAI tools via `pip install nodeproxy-tools[langchain]`. ([Health](https://nodeproxy-production.up.railway.app/health) | [MCP tools](https://nodeproxy-production.up.railway.app/mcp/tools) | [PyPI](https://pypi.org/project/nodeproxy-tools/) | [GitHub](https://github.com/pgalyen1987/NodeProxy))

- [Gate402](https://gate402.app) - The programmable compute + data market for AI agents over x402 (USDC on Base/Polygon/Arbitrum/Solana). One 402 flow to discover, pay for, and use: per-token open-model LLM inference (Llama 3.1 8B/70B, Qwen 2.5, Mistral), per-second GPU/CPU container compute, on-chain/DeFi market data (wallet/token intel, DEX price/liquidity, token-risk/honeypot sim, momentum, best-swap, launch radar), SEC-EDGAR filings, news+sentiment, and URL→clean-Markdown scraping (incl. Cloudflare-stealth). Signed execution/usage receipts, no signup. Also an MCP server (`npx -y gate402-mcp`). ([Catalog](https://gate402.app/v1) | [Discovery](https://gate402.app/.well-known/x402.json) | [llms.txt](https://gate402.app/llms.txt))
