...
- [x402lab](https://api.x402lab.dev/v1/catalog) - XRPL AMM risk data for LP and DeFi research agents: minute-cadence pool snapshots, impermanent-loss/fee decomposition, and stale-bias-corrected volatility (Hansen-Lunde realized kernel) from a continuously collected XRP/RLUSD observation archive. Settles on the XRP Ledger via the t54 XRPL facilitator — pay per call in XRP or RLUSD (0.01-0.09 XRP / 0.011-0.099 RLUSD). No API keys, no signup. ([Discovery](https://api.x402lab.dev/.well-known/x402)) ([Docs/Catalog](https://api.x402lab.dev/v1/catalog))
