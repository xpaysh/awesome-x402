...
- [Kraken Crypto Signals](https://signals.nsgoods.org) — Pay-per-call crypto market-state signals (BUY/SELL/HOLD + confidence) for AI trading agents, from a live multi-timeframe Kraken strategy. 6 endpoints on Base mainnet: `/signals` ($0.01), `/signals/top` ($0.10), `/indicators` — RSI/EMA/MACD/ATR/Bollinger ($0.005), `/regime/changed` — RISK_ON/RISK_OFF ($0.02), `/history` — revealed snapshots + verifiable hashes ($0.05), `/backtest` — walk-forward win-rate ($0.25). Every response ECDSA-signed (verify `signed_by`). Free preview + health. USDC via Coinbase CDP Facilitator, no API keys. ([Manifest](https://signals.nsgoods.org/.well-known/x402)) ([Preview](https://signals.nsgoods.org/signals/preview)) ([GitHub](https://github.com/Nikoble1926/kraken-crypto-signals)) ([MCP](https://www.npmjs.com/package/kraken-signals-mcp))
...
- [Agent Trust Oracle](https://trust.nsgoods.org) — Pay-per-call ERC-8004 trust scores for AI agents across 5 EVM chains (Ethereum, Base, BSC, Mantle, Polygon). Reads on-chain Identity + Reputation registries and returns a transparent 0–100 score with component breakdown (client breadth, recency, value, volume) + methodology link. `/agent-trust` ($0.005 USDC on Base), free `/agent-trust/preview`. ECDSA-signed; tamper-evident snapshot chain mirrored to GitHub. ([Manifest](https://trust.nsgoods.org/.well-known/x402)) ([Methodology](https://trust.nsgoods.org/methodology)) ([Preview](https://trust.nsgoods.org/agent-trust/preview)) ([GitHub](https://github.com/Nikoble1926/agent-trust-oracle))
