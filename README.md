...
- [agent402-client](https://www.npmjs.com/package/agent402-client) - Non-custodial buyer SDK for x402 + MPP (the buy side of Agentic Finance (AIFI)). Two methods: `find()` resolves a natural-language task to a paid tool, `call()` invokes it with automatic dual-rail payment (free via proof-of-work on pure-CPU tools, paid via x402 USDC on 12 chains, or over MPP with a stock mppx fetch). Built-in caching, idempotent retries (`Idempotency-Key`), and Bazaar-shape 402 challenge handling. Works against any Agent402-compatible server. ([GitHub](https://github.com/MikeyPetrillo/Agent402/tree/main/client))
...
- [Agent402 MCP](https://agent402.tools/mcp) - Hosted MCP connector (the applied layer of Agentic Finance (AIFI); the API behind it answers x402 and MPP on the same 402) for the Agent402 server (500+ deterministic tools). Four meta-tools — `search_tools`, `find_tool`, `call_tool`, `about_agent402` — let an agent discover and invoke the full catalog without bloating its tool list. Dual-rail payment: free via proof-of-work for pure-CPU tools, paid via x402 on 10 chains for the rest. No LLM in the serving path. Categories: browser, search, PDF/image/OCR/audio, geo, live data (FX, weather, USGS, gov-data), network truth (DNS/TLS/WHOIS), crypto/x402 helpers, 200+ pure-CPU utilities. Self-hostable; also distributed as the [`agent402-mcp`](https://www.npmjs.com/package/agent402-mcp) stdio package and registered with the official MCP Registry as `io.github.MikeyPetrillo/agent402`. ([GitHub](https://github.com/MikeyPetrillo/Agent402)) ([OpenAPI](https://agent402.tools/openapi.json)) ([Discovery](https://agent402.tools/.well-known/x402))
...
- [agent402-langchain](https://pypi.org/project/agent402-langchain/) - LangChain & CrewAI tools for [Agent402](https://agent402.tools)'s catalog (the applied layer of Agentic Finance (AIFI); pay over x402, or MPP with a stock mppx fetch). Exposes four meta-tools (find / route / call / about) instead of thousands, so the LLM's tool list stays small; pure-CPU tools settle for free via built-in proof-of-work, wallet-only tools via your x402 fetch. `pip install agent402-langchain`. ([PyPI](https://pypi.org/project/agent402-langchain/)) ([Source](https://github.com/MikeyPetrillo/Agent402/tree/main/adapters/langchain-py))
