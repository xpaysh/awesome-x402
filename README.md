...
- [true402](https://true402.dev) - Machine-native marketplace on Base mainnet with 12 agent-payable stalls across on-chain safety/DeFi (token rug + buy/sell **honeypot simulation**, address safety + proxy detection, composite token report, new-pair / liquidity-pull / whale feeds), web (extract, SEO-GEO audit, link preview, robots + security-headers), and LLM inference. $0.0005–$0.015 USDC/call. No accounts, no API keys, no KYC — wallet is identity, settled through a self-hosted no-KYC facilitator. Verified Base Builder. ([Discovery](https://true402.dev/.well-known/x402) | [OpenAPI](https://true402.dev/openapi.json) | [MCP](https://true402.dev/.well-known/mcp.json) | [llms.txt](https://true402.dev/llms.txt) | [GitHub](https://github.com/true402/mcp-server))
