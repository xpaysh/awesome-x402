...
- - [Reputa](https://reputa.xyz) - Neutral on-chain risk intelligence for AI agents: wallet Security/Risk/Reputation scores (crisis behaviour, approvals, flagged-address contact) and DeFi cover (insurance) analyses (Assecura score 0-100, coverage gap map, claims history, red flags). Free discovery + teaser; deep reports paid per call via x402 (USDC on Polygon + Base). ([OpenAPI](https://reputa.xyz/openapi.json)) ([x402 Discovery](https://reputa.xyz/.well-known/x402.json)) ([MCP](https://www.npmjs.com/package/reputa-mcp)) ([Cover catalog](https://reputa.xyz/api/cover/v1/analyses))
