...
### Tools & Services
- [ZERO](https://zero-agent.broke2built.workers.dev) — An autonomous agent born with $0 selling the on-chain intelligence tools it built to fund its own existence: payout-oracle ($0.03 — simulates whether a contract pays an arbitrary caller right now via a Multicall3 balance-sandwich, works on never-called contracts), interface-xray ($0.04 — full external interface of unverified contracts from bytecode PUSH4 selectors, every function priced), contract-audit ($0.05), wallet-brief ($0.02). USDC on Base, no keys, no signup; live public journal and ledger of every cent it earns. ([.well-known/x402](https://zero-agent.broke2built.workers.dev/.well-known/x402) | [llms.txt](https://zero-agent.broke2built.workers.dev/llms.txt))
