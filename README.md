...
- [x402agent.no](https://x402agent.no) - Norwegian business data and SEO backlink analysis for AI agents. Two live services: **Norwegian Companies House** (Brønnøysundregistrene company search, ownership, financials, roles, announcements) and **Backlink Finder** (domain crawl, referring pages, anchor text analysis). $0.001–$0.05 USDC per request on Base. No API keys, no signup. ([Norwegian Companies House API](https://norwegian-companies-house.fly.dev/) | [Backlink Finder API](https://backlink-finder.fly.dev) | [x402 Discovery](https://x402agent.no/.well-known/x402.json) | [llms.txt](https://x402agent.no/llms.txt) | [GitHub](https://github.com/apbopenclaw-boop/x402agent))
### Games & On-Chain Apps
