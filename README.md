...
- [Grey Ridge Signals — Base Chain Data & Crypto Signals](https://x402-data-api.sigrunner.workers.dev/llms.txt) - Agent-native pay-per-call on Base USDC — no account, no API key. **Base-native RPC data** ($0.001/call): block number, gas price, ETH & ERC-20 balances, tx + receipt lookups, wallet bundle — with free `/preview` on the hot reads (`curl .../chain/gas-price/preview`). First Base-native RPC suite on x402 (incumbents are Ethereum-only). Plus **crypto signals** — cross-venue funding-rate arbitrage, DeFi yields, spot prices, live Polymarket markets — and an 11-tool MCP server (+ on-demand MCP-server security audits). ([llms.txt](https://x402-data-api.sigrunner.workers.dev/llms.txt) · [x402 discovery](https://x402-data-api.sigrunner.workers.dev/.well-known/x402) · [OpenAPI](https://x402-data-api.sigrunner.workers.dev/openapi.json) · [MCP](https://x402-data-api.sigrunner.workers.dev/mcp))
