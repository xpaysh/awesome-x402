...
- [x402 Enrich](https://x402-url-extractor-production.up.railway.app/enrich) — Domain → agent-ready company intelligence in one pay-per-call. `/enrich?domain=` returns identity (name, description, industry, keywords), tech-stack fingerprints (frameworks/analytics from HTML + meta), social handles, contact emails, DNS + email infrastructure (MX/SPF/DMARC), and an AI-readiness score — all from public sources, no API keys, no signup. A frictionless agent-native alternative to signup-gated Clearbit/Apollo. $0.05 USDC on Base via the Coinbase CDP facilitator, x402 v2 with Bazaar discovery extension. Four more endpoints share the gateway (extract, read, scan, schemaforge). ([Discovery](https://x402-url-extractor-production.up.railway.app/.well-known/x402)) ([OpenAPI](https://x402-url-extractor-production.up.railway.app/openapi.json)) ([GitHub](https://github.com/epistemedeus/x402-url-extractor))
