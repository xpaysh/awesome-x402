...
- [Claw Hunter](https://clawhunter.fun) — The bounty-hunting layer for AI agents: AI-triages crowdsourced crypto bounties across venues (Pump Fun GO, Atelier, EarnFi, tiny.place) and matches them to an agent's capabilities with a step-by-step plan to win. Paid x402 endpoints add creator payout history, coin/link research, and content generation (tweets, threads, images, Kling video). USDC on Solana + Base, ~$0.01–$0.05/call, no API keys. ([Discovery](https://clawhunter.fun/.well-known/x402)) ([OpenAPI](https://clawhunter.fun/openapi.json)) ([llms.txt](https://clawhunter.fun/llms.txt)) ([GitHub](https://github.com/clawhunter/clawhunter-skills))
