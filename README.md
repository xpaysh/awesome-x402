...
### Tools & Services
- [StockWaves](https://stockwaves.net) — China & global market data + quant signals + a tokenized-stock pre-trade safety gate, all x402 pay-per-call (USDC on Base + Solana, no API key). 13 paid endpoints: real-time Weibo/Baidu/Douyin hot-search pulse, structured CN news/events + sector-theme momentum, quant signals across A-share/HK/US equities + crypto + macro, and an "is this the REAL token vs a copycat?" gate (depeg/liquidity/market-hours/freeze-authority) across Backed/Ondo/Remora/Backpack on Solana + EVM. Free walk-forward track-record + safety preview. Indexed on [402index.io](https://402index.io). ([llms.txt](https://stockwaves.net/llms.txt) | [OpenAPI](https://stockwaves.net/openapi.json))
- [402Sentinel](https://402sentinel.com) — Pre-payment counterparty risk + compliance gate for AI agents. 7 paid endpoints: seller risk score (allow/review/block + key_factors), a buyer-side payment firewall (prompt-injection-destination + intent-mismatch hard blocks), OFAC/FATF compliance screening, and a tokenized-RWA transfer-restriction gate. x402 USDC on Base + Solana; free preview + public verified allow-list. Indexed on [402index.io](https://402index.io). ([llms.txt](https://402sentinel.com/llms.txt) | [OpenAPI](https://402sentinel.com/openapi.json))
