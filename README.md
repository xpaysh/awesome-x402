...
- [@magpieloans/magpie-agent](https://www.npmjs.com/package/@magpieloans/magpie-agent) - SDK for **Magpie**, the first x402-native permissionless **lending** protocol. An agent takes a permissionless loan against its own assets — borrow SOL against memecoin or tokenized-RWA collateral, then optionally arm self-owned in-vault take-profit / stop-loss exits on the loan and repay. The whole leveraged-position lifecycle, each action a typed one-liner; the SDK handles the x402 payment + signing. No signup, no API key, zero custody (the service holds no keys — the agent signs locally). 70% of x402 protocol fees accrue to $MAGPIE holders. Solana. `npm i @magpieloans/magpie-agent`. ([GitHub](https://github.com/magpiecapital/magpie-x402))
...
- [@magpieloans/magpie-mcp](https://www.npmjs.com/package/@magpieloans/magpie-mcp) - MCP server for **Magpie**, the first x402-native permissionless **lending** protocol on Solana. Your agent takes a permissionless loan against its own assets (memecoins / tokenized RWAs), arms self-owned in-vault take-profit / stop-loss exits on the loan, and reads pool / loan / credit-score state — all as native tools in Claude Desktop / Cursor / Windsurf / ChatGPT. Free read tools; paid writes (borrow 0.005 SOL, arm exit 0.001 SOL) sign locally with your own keypair — zero custody, the service holds no keys. 70% of x402 protocol fees accrue to $MAGPIE holders. `npx -y @magpieloans/magpie-mcp`. ([GitHub](https://github.com/magpiecapital/magpie-x402)) ([Discovery](https://x402.magpie.capital/.well-known/x402.json))
