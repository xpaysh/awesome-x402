...
- [tokenbrief](https://89.106.83.95.sslip.io) - Crypto token research API for AI agents, pay-per-call in USDC on Base via x402. `GET /token/{chain}/{address}` token overview ($0.05), `/trending/{chain}` trending tokens ($0.02), `/wallet/{chain}/{address}` wallet holdings ($0.02), `/report/{chain}/{address}` due-diligence report (GoPlus security scan + market data, $0.50). No API keys, no signup. ([OpenAPI](https://89.106.83.95.sslip.io/openapi.json))
...
- [llmgate](https://89.106.83.95.sslip.io:8443) - OpenAI-compatible LLM chat completions with per-request x402 payment (USDC on Base). `POST /v1/chat/completions` at $0.01 per request, no API keys, no signup.
