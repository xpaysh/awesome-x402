...
20 developer utility tools for AI agents and automation pipelines, all on Base mainnet. Self-hosted, no API keys, no signup. $0.0005–$0.001 USDC/call.

- [x402 Hash Calculator](https://x402-hash.suretat.com) — Multi-algorithm hashing: MD5, SHA-1, SHA-224, SHA-256, SHA-384, SHA-512, SHA3-256, SHA3-512, BLAKE2b, BLAKE2s. `POST /hash` with text or binary input. 0.0005 USDC/call. ([Docs](https://x402-hash.suretat.com/docs) | [/.well-known/x402.json](https://x402-hash.suretat.com/.well-known/x402.json))
- [x402 Markdown to HTML](https://x402-markdown.suretat.com) — Markdown to HTML with GFM support (tables, strikethrough, task lists, autolinks). `POST /convert`. 0.0005 USDC/call. ([Docs](https://x402-markdown.suretat.com/docs) | [/.well-known/x402.json](https://x402-markdown.suretat.com/.well-known/x402.json))
- [x402 Regex Tester](https://x402-regex.suretat.com) — Regex operations: `POST /test` (match + groups), `POST /extract` (all matches), `POST /replace`, `POST /split`. Python/PCRE flags supported. 0.0005 USDC/call. ([Docs](https://x402-regex.suretat.com/docs) | [/.well-known/x402.json](https://x402-regex.suretat.com/.well-known/x402.json))
- [x402 Luhn Validator](https://x402-luhn.suretat.com) — Luhn checksum validation for credit cards, IMEI, and any Luhn-encoded ID. `POST /validate`, `POST /generate` (valid test numbers). 0.0005 USDC/call. ([Docs](https://x402-luhn.suretat.com/docs) | [/.well-known/x402.json](https://x402-luhn.suretat.com/.well-known/x402.json))
- [x402 Unit Converter](https://x402-units.suretat.com) — Unit conversion across 10 categories: length, mass, temperature, volume, area, speed, time, energy, pressure, data storage. `POST /convert`. 0.0005 USDC/call. ([Docs](https://x402-units.suretat.com/docs) | [/.well-known/x402.json](https://x402-units.suretat.com/.well-known/x402.json))
- [x402 Timezone Converter](https://x402-timezone.suretat.com) — DST-aware timezone conversion. `POST /convert` (datetime + source → target IANA zone), `POST /list`, `POST /offset`. 0.0005 USDC/call. ([Docs](https://x402-timezone.suretat.com/docs) | [/.well-known/x402.json](https://x402-timezone.suretat.com/.well-known/x402.json))
- [x402 Cron Parser](https://x402-cron.suretat.com) — Cron expression analysis: `POST /parse` (next N run times + human description), `POST /validate`, `POST /next`. Supports 5-field and 6-field (with seconds). 0.0005 USDC/call. ([Docs](https://x402-cron.suretat.com/docs) | [/.well-known/x402.json](https://x402-cron.suretat.com/.well-known/x402.json))
- [x402 Base Encoder](https://x402-base64.suretat.com) — Encoding/decoding for Base64, Base64url, Base32, Base16 (hex), and URL encoding. `POST /encode`, `POST /decode`. UTF-8 and binary support. 0.0005 USDC/call. ([Docs](https://x402-base64.suretat.com/docs) | [/.well-known/x402.json](https://x402-base64.suretat.com/.well-known/x402.json))
- [x402 UUID Generator](https://x402-uuid.suretat.com) — UUID generation (v1/v4/v5/v7) with `POST /generate` (up to 100 at once), `POST /validate`, `POST /parse`. 0.0005 USDC/call. ([Docs](https://x402-uuid.suretat.com/docs) | [/.well-known/x402.json](https://x402-uuid.suretat.com/.well-known/x402.json))
- [x402 QR Code Generator](https://x402-qrcode.suretat.com) — QR code generation: PNG or SVG output, configurable error correction (L/M/Q/H), size, and colors. `POST /generate`. 0.0005 USDC/QR. ([Docs](https://x402-qrcode.suretat.com/docs) | [/.well-known/x402.json](https://x402-qrcode.suretat.com/.well-known/x402.json))
- [x402 Barcode Generator](https://x402-barcode.suretat.com) — Barcode generation: EAN-13, EAN-8, Code 128, Code 39, UPC-A, ITF, QR. PNG or SVG. `POST /generate`. 0.0005 USDC/barcode. ([Docs](https://x402-barcode.suretat.com/docs) | [/.well-known/x402.json](https://x402-barcode.suretat.com/.well-known/x402.json))
- [x402 Color Palette Extractor](https://x402-colorpalette.suretat.com) — Dominant color extraction from any image URL. Returns hex, RGB, HSL, color names by dominance rank. Configurable palette size (1–20). `POST /extract`. 0.001 USDC/image. ([Docs](https://x402-colorpalette.suretat.com/docs) | [/.well-known/x402.json](https://x402-colorpalette.suretat.com/.well-known/x402.json))
- [x402 Text Statistics](https://x402-textstats.suretat.com) — Text analysis: word/character/sentence count, readability scores (Flesch, Kincaid, Gunning Fog), lexical density, top keywords. `POST /analyze`. 0.0005 USDC/call. ([Docs](https://x402-textstats.suretat.com/docs) | [/.well-known/x402.json](https://x402-textstats.suretat.com/.well-known/x402.json))
- [x402 RSS Feed Parser](https://x402-rss.suretat.com) — RSS/Atom feed parsing: structured feed metadata + items (title, link, date, description, author). Supports RSS 2.0, RSS 1.0, Atom. `POST /parse`. 0.001 USDC/feed. ([Docs](https://x402-rss.suretat.com/docs) | [/.well-known/x402.json](https://x402-rss.suretat.com/.well-known/x402.json))
- [x402 JSON Schema Validator](https://x402-jsonschema.suretat.com) — JSON Schema validation (Draft-07/2020-12): `POST /validate` (pass/fail + error paths), `POST /generate` (schema from example). 0.0005 USDC/call. ([Docs](https://x402-jsonschema.suretat.com/docs) | [/.well-known/x402.json](https://x402-jsonschema.suretat.com/.well-known/x402.json))
- [x402 JWT Decoder](https://x402-jwt.suretat.com) — JWT inspection: `POST /decode` (header + payload), `POST /verify` (HMAC/RSA/ECDSA), `POST /inspect` (expiry, issuer, claims). No key stored server-side. 0.0005 USDC/call. ([Docs](https://x402-jwt.suretat.com/docs) | [/.well-known/x402.json](https://x402-jwt.suretat.com/.well-known/x402.json))
- [x402 Password Tools](https://x402-password.suretat.com) — Password utilities: `POST /generate` (configurable charset/length/entropy), `POST /strength` (zxcvbn score + crack time), `POST /entropy`. 0.0005 USDC/call. ([Docs](https://x402-password.suretat.com/docs) | [/.well-known/x402.json](https://x402-password.suretat.com/.well-known/x402.json))
- [x402 Currency Converter](https://x402-currency.suretat.com) — Exchange rates and conversion for 170+ currencies. `POST /convert`, `GET /rates` (vs base), `GET /history` (historical date). 0.0005 USDC/call. ([Docs](https://x402-currency.suretat.com/docs) | [/.well-known/x402.json](https://x402-currency.suretat.com/.well-known/x402.json))
- [x402 Subnet Calculator](https://x402-subnet.suretat.com) — IPv4/IPv6 subnet analysis. `POST /calculate` returns network address, broadcast, usable range, CIDR, host count. `POST /contains` checks if IP is within CIDR. 0.0005 USDC/call. ([Docs](https://x402-subnet.suretat.com/docs) | [/.well-known/x402.json](https://x402-subnet.suretat.com/.well-known/x402.json))
- [x402 URL Metadata Extractor](https://x402-urlmeta.suretat.com) — Web page metadata extraction: title, description, Open Graph, Twitter Card, canonical URL, JSON-LD, favicon, main text. `POST /extract`. 0.001 USDC/URL. ([Docs](https://x402-urlmeta.suretat.com/docs) | [/.well-known/x402.json](https://x402-urlmeta.suretat.com/.well-known/x402.json))
