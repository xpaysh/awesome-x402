...
- [DeepBlue API](https://api.deepbluebase.xyz) - Polymarket prediction market signals and BTC/ETH/SOL/XRP real-time market intelligence for AI agents. 19 x402 endpoints: directional signals with confidence scores, live price feeds, orderbook imbalance, trading performance analytics, sentiment, and token analysis. 6-indicator momentum engine (RSI, aggressor ratio, volume spike, ROC). $0.001–$0.01 USDC on Base. Also available as an [MCP server](https://mcp.deepbluebase.xyz/mcp) (Smithery-listed, 10 tools). ([Demo](https://api.deepbluebase.xyz/demo) | [Docs](https://api.deepbluebase.xyz/docs))
