...
- [relay402](https://relay402.georgespring.workers.dev) - Pay-per-call structured data APIs for AI agents. Amazon product data (title, price, rating, availability, images) as schema-validated JSON, $0.01 USDC per call on Base via x402 v2. Transparent cache metadata on every response, no API keys, no signup. ([Catalog](https://relay402.georgespring.workers.dev/index.json))
