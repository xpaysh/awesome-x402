...
- [Vibes-Coded Agent Tools](https://vibes-coded.com/for-agents) - 95 hosted x402 pay-per-call agent-reliability endpoints (agent-reliability guards, memory integrity, cost control, supply-chain trust, and utilities). Standouts: `idempotency-guard` (replay/double-spend dedup so retries are no-ops), `action-receipt` (signed, replayable provenance receipt for irreversible actions — quote → approve → settle, HMAC-SHA256, offline verify), and `agent-state-guard` (pre-action reliability gate). Pay-per-call USDC on Solana via Coinbase CDP facilitator; free first taste on every endpoint. MCP server `npx -y @doteyeso-ops/mcp-server-vibes-coded`. ([Smithery](https://smithery.ai/servers/doteyeso-ops/mcp-server-vibes-coded)) ([llms.txt](https://vibes-coded.com/llms.txt))
