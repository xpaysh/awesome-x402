...
- [Losbeto](https://losbeto-production-dd7c.up.railway.app) - Multi-chain x402 intelligence node with global markets (not crypto-only): forex (EUR/USD, GBP/USD, USD/JPY), commodities (gold, oil, silver), stocks (AAPL/TSLA/NVDA), and macro calendar (FOMC/NFP/ECB) alongside crypto data. 60 endpoints, $0.003–$0.99 USDC on Solana + Base, dual facilitator (PayAI + Coinbase CDP). Flagship AI products: Launch Risk Brief (on-chain + DEX + AI verdict) and Council of Five (5 AI agents vote + chairman synthesis). Free delayed preview on every endpoint (`?preview=1`), prepaid credits, MCP server, and every sale labeled test-vs-organic on-chain at /receipts. ([Manifest](https://losbeto-production-dd7c.up.railway.app/.well-known/x402.json)) ([MCP](https://www.npmjs.com/package/losbeto-mcp)) ([Receipts](https://losbeto-production-dd7c.up.railway.app/receipts))
