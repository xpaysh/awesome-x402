...
- [Superhighway MCP](https://github.com/patwalls/superhighway-mcp) - Web search for AI agents, paid per call in USDC on Base via x402 — no API key, no signup. Five tools that do the whole search job: `web_search` (live ranked results), `news_search` (recent articles), `image_search` (image URLs + thumbnails), `scrape` (read any page as clean markdown), `research` (search + read in one call). On the official MCP registry (`io.github.patwalls/superhighway-mcp`). Install: `npx -y superhighway-mcp`. Free no-wallet trial: [superhighway.walls.sh](https://superhighway.walls.sh)
