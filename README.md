...
- [ToolSnap MCP](https://mcp.toolsnap.app) — 33 MCP tools for AI agents on Cloudflare Workers (edge-native, 0ms cold start). Flagship `fetch_extract` benchmarked at **98.1% median token reduction** (53,820 → 2,001 tokens), saving ~$0.156/call in context costs at Sonnet pricing. Paid tools: web extraction, screenshot, background removal (fal.ai rembg), keyword research (DataForSEO), PDF, HTML-to-Markdown, page inventory, file upload. 20 always-free utility tools (UUID, hash, Base64, JSON, timestamps, regex…). x402 v2 (EIP-3009 `transferWithAuthorization`) on Base mainnet — $0.02/call or $0.01 prepaid (deposit ≥$0.50 USDC, off-chain debit, no per-call gas). Includes `pay-proxy`: local stdio bridge that auto-signs 402 challenges so any MCP client gains payment capability without code changes. Machine-readable: [`/.well-known/mcp.json`](https://mcp.toolsnap.app/.well-known/mcp.json) · [`/.well-known/pricing.json`](https://mcp.toolsnap.app/.well-known/pricing.json). ([GitHub](https://github.com/icosaedro-git/toolsnap-mcp) · [Smithery](https://smithery.ai/servers/icosaedro/toolsnap-mcp) · [MCP Registry](https://registry.modelcontextprotocol.io/servers/app.toolsnap-toolsnap-mcp))
