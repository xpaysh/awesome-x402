...
- [Seneschal](https://seneschal.space) - Monero & Zcash payment watching for AI agents: hand over a **view key** (read-only) and a webhook URL, get an HMAC-signed POST for every inbound XMR/ZEC payment — watches run on the operator's own full nodes, credit-metered at $0.02/day + $0.005/event, topped up via x402 USDC **or natively in XMR/ZEC**. New: **Seneschal Pay**, a non-custodial XMR/ZEC checkout — exact-amount invoices paid straight to *your* wallet, hosted QR pay page, one-script-tag embed button, `invoice_paid` webhooks; creation free, flat $0.02 per settled sale (the "private payment option next to Stripe"). Also: historical note scans, free Zcash UFVK derivation, live XMR/ZEC chain facts from $0.001, per-block base+blob fee history, DeFi liquidation telemetry (Aave, Morpho, Spark, Compound) and Ethereum builder bid distributions. Settles via Coinbase CDP with Bazaar discovery on every route. REST + MCP, no API keys. ([Discovery](https://api.seneschal.space/.well-known/x402)) ([MCP Server](https://mcp.seneschal.space)) ([Checkout](https://api.seneschal.space/v1/checkout)) ([GitHub](https://github.com/Rotwang9000/seneschal-data-api))
