...
- [OpenVecta](https://openvecta.com) - OpenAI-compatible LLM inference gateway with accountless x402 pay-per-call (USDC on Solana). No signup, no API key — agents pay per request straight from their wallet. Three tiers from frontier to eco: Kimi K3, Claude Sonnet 4.6, GLM-5.2, Qwen3, DeepSeek, Llama and more, at per-token pricing + a flat $0.002/call. ([Discovery](https://api.openvecta.com/.well-known/x402)) ([Docs](https://openvecta.com/docs)) ([MCP](https://www.npmjs.com/package/@openvecta/mcp))
