...
- [Social Intel](https://socialintel.dev) — Instagram influencer search API for AI agents. Keyword-based discovery across 100M+ profiles with filters for country, gender, city, followers range, and category. Returns usernames, follower counts, emails, and verified status. Free demo endpoint included. $0.10 USDC on Base. MCP server available. ([MCP](https://smithery.ai/server/@socialinteldev/social-intel-mcp) | [Docs](https://socialintel.dev))
