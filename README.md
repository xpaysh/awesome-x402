...
### Data & Social APIs
- [PulseNetwork](https://www.npmjs.com/package/mcp-pulsenetwork) - 66 specialized intelligence APIs (660+ endpoints) for AI agents, pay-per-call in USDC on **Base and Solana** via x402. Domains span finance, crypto, legal, immigration, healthcare cost, real estate, tax, climate, sports, and science — $0.07–$0.50 per call, no signup. One MCP server (`npx mcp-pulsenetwork`) exposes all of them, and every response carries a machine-readable `network_referrals` graph so an agent can chain from any endpoint to the most relevant sibling. ([MCP server](https://www.npmjs.com/package/mcp-pulsenetwork)) ([Discovery](https://immigrationpulse.vercel.app/.well-known/agent.json)) ([OpenAPI](https://immigrationpulse.vercel.app/openapi.json))
