...
### French Government & Administrative Data APIs

Seven x402-powered APIs for French administrative data, all live on Base mainnet. Official open data sources (data.gouv.fr, INSEE, BAN).

- [x402 INSEE Statistiques FR](https://x402-insee.suretat.com) — French national statistics via the official INSEE API. Endpoints: `POST /population` (commune population by year), `POST /employment` (unemployment rate by zone/sex/age), `POST /birth-rate`, `POST /death-rate`. Data from official INSEE series. 0.002 USDC/call. ([Docs](https://x402-insee.suretat.com/docs) | [/.well-known/x402.json](https://x402-insee.suretat.com/.well-known/x402.json))
- [x402 Phone Validator FR](https://x402-phone-fr.suretat.com) — French phone number validation and enrichment. Returns E.164/international/national formats, line type (mobile/fixe/VoIP), carrier, region, and timezone. RFC-compliant. 0.0005 USDC/call. ([Docs](https://x402-phone-fr.suretat.com/docs) | [/.well-known/x402.json](https://x402-phone-fr.suretat.com/.well-known/x402.json))
- [x402 VAT Calculator FR](https://x402-vat-fr.suretat.com) — French VAT calculation: `POST /calculate` (single amount with all applicable rates), `POST /invoice` (multi-line invoice with per-line and totals breakdown), `GET /rates` (full schedule of French VAT rates). 0.0005 USDC/call. ([Docs](https://x402-vat-fr.suretat.com/docs) | [/.well-known/x402.json](https://x402-vat-fr.suretat.com/.well-known/x402.json))
- [x402 Jours Fériés FR](https://x402-feries.suretat.com) — French public holidays API. `POST /feries` returns all public holidays for a given year, `POST /jours-ouvres` computes working days between two dates. Covers all 11 French administrative zones including overseas territories (Guadeloupe, Martinique, Guyane, Réunion, Mayotte, Alsace-Moselle, Saint-Barthélemy, Saint-Martin, Saint-Pierre-et-Miquelon, Wallis-et-Futuna, Polynésie, Nouvelle-Calédonie). 0.0005 USDC/call. ([Docs](https://x402-feries.suretat.com/docs) | [/.well-known/x402.json](https://x402-feries.suretat.com/.well-known/x402.json))
- [x402 Fake Data FR](https://x402-fakedata-fr.suretat.com) — French locale synthetic test data generator. `POST /generate` with profiles: `personne` (French names, addresses, SIRET, phone), `entreprise` (company with SIREN, NAF code, address), `adresse` (realistic French postal addresses), `paiement` (IBAN, cards, amounts), `internet`, `texte`. Up to 20 records per call. 0.001 USDC/call. ([Docs](https://x402-fakedata-fr.suretat.com/docs) | [/.well-known/x402.json](https://x402-fakedata-fr.suretat.com/.well-known/x402.json))
- [x402 Nombre en Lettres](https://x402-num2words.suretat.com) — Number-to-words conversion in 13 languages (fr, fr_BE, fr_CH, en, de, es, it, pt, nl, pl, ru, ar, ja). `POST /convert` accepts integers and decimals, returns spelled-out number and optionally currency format. 0.0005 USDC/call. ([Docs](https://x402-num2words.suretat.com/docs) | [/.well-known/x402.json](https://x402-num2words.suretat.com/.well-known/x402.json))
- [x402 PDF Generator FR](https://x402-pdf-generator.suretat.com) — Server-side PDF generation with French locale support. `POST /invoice` generates invoice PDFs from JSON (line items, VAT, IBAN), `POST /html-to-pdf` converts HTML/CSS to PDF with full Unicode and French typography. 0.005 USDC/PDF. ([Docs](https://x402-pdf-generator.suretat.com/docs) | [/.well-known/x402.json](https://x402-pdf-generator.suretat.com/.well-known/x402.json))
