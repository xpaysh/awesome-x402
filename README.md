...
-  **Refiner Gateway** — CVE → grounded fix remediation for agents: fixed version, upgrade path, breaking-change risk, migration notes. Grounded on OSV, cached. Pay-per-call USDC on Base ($0.02/call, no account, no key). [Live](https://refiner-gateway.onrender.com) · [GitHub](https://github.com/keich2realg/refiner-gateway)
