...
- [PayAPI Market](https://payapi.market) - x402 API marketplace where listings are verified by an actual paid call. The operator pays each listing from his own wallet on Base mainnet and records the settlement transaction hash, rather than scoring endpoints heuristically. 27 live listings from 14 independent providers, 25 currently carrying the payment-verified badge. Coverage spans UK and Dutch public data, US business-registry and compliance evidence, crypto and DeFi market data, on-chain risk and sanctions screening, market and news feeds, and agent tooling. Pay per request in USDC on Base, from $0.001 to $1.77. MCP server discovery. ([GitHub](https://github.com/chetparker/x402-marketplace))
