...
- [Suede AI](https://app.suedeai.ai) - Rights-aware music and short-video generation for AI agents. 17 paid endpoints including `POST /create-music` ($0.75 USDC/track) and `POST /agent/video` ($5.00 USDC/clip). Outputs are fingerprinted for on-chain provenance attestation via the Suede Registry on Base. ERC-8004 agent card published. ([Discovery](https://app.suedeai.ai/.well-known/x402.json)) ([Agent Card](https://app.suedeai.ai/.well-known/agent-card.json))
