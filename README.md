...
- [Superhighway MCP](https://github.com/patwalls/superhighway-mcp) - A toolkit of 14 web tools any MCP agent pays for per call in USDC on Base via x402 — no API key, no signup. Tools: search, news, scrape, geocode, nlp, email-verify, convert, qr, feed, sitemap, unfurl, fx (currency), dns, hash. On the official MCP registry (`io.github.patwalls/superhighway-mcp`). Install: `npx -y superhighway-mcp`. Free no-wallet trial: [superhighway.walls.sh](https://superhighway.walls.sh)
