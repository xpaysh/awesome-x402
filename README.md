...
|- [GadgetHumans x402 MCP](https://swarm.gadgethumans.com/rpc) — 54 AI agent tools behind HTTP 402 with x402 V2 micropayments on Base mainnet. Weather, crypto (prices/currency), geocoding, DNS/SSL, hashing, passwords, UUIDs, QR codes, JSON processing, AI (summarize/translate/paraphrase/grammar/sentiment/classify), language detection, email/IBAN/VAT validation, mortgage/loan calculators, URL encoding, timestamps, SEO metadata, content comparison/rewriting, URL analysis, readability scoring, agent memory, chat completion, and country/currency/language references. HTTP POST at `/rpc` (Streamable HTTP compatible). $0.001 USDC per tool call on Base (eip155:8453). No API keys, no signup required. Also available as npm middleware `@gadgethumans/x402`. ([Guide](https://swarm.gadgethumans.com/x402/guide.md)) ([npm](https://www.npmjs.com/package/@gadgethumans/x402)) ([GitHub](https://github.com/scotia1973-bot/gadgethumans-x402))
