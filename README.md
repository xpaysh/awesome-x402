...
### Data & Social APIs
- [x402 OSINT](https://x402-osint.tail66f665.ts.net) - Pay-per-call OSINT for AI agents. `GET /osint/{username}` ($0.02, 500+ sites via maigret), `GET /report/{username}` ($0.05, profiles + candidate emails + breach exposure + findability score), and `GET /email/{address}` ($0.03, reverse email across ~120 services via holehe). Free `/trial` endpoint, no API keys, USDC on Base via CDP facilitator with Bazaar discovery. Every response includes an LLM-ready `summary` field. ([llms.txt](https://x402-osint.tail66f665.ts.net/llms.txt)) ([OpenAPI](https://x402-osint.tail66f665.ts.net/openapi.json)) ([MCP](https://x402-osint.tail66f665.ts.net:8443/sse))
