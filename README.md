...
- [SameDayDesk Repo Security Scan](https://x402-url-extractor-production.up.railway.app/scan) - Static supply-chain security scan of a public GitHub repo BEFORE an agent installs or runs it (a dependency, a Claude/MCP skill, or an MCP server). Flags exfil sinks, obfuscated code execution, credential-file reads, env-harvest+network, and install-time `curl|bash`; returns `clean`/`suspicious`/`dangerous` plus findings. Static only, never runs the code; tuned for low false positives. $0.02 USDC on Base mainnet, no API keys. ([/.well-known/x402](https://x402-url-extractor-production.up.railway.app/.well-known/x402)) ([OpenAPI](https://x402-url-extractor-production.up.railway.app/openapi.json)) ([GitHub](https://github.com/epistemedeus/x402-url-extractor))
