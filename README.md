...
- [x402guard](https://x402guard.io) - Security scanner for x402 / agent-payment **integration code** — finds the web↔chain-glue bugs general Solidity scanners (Slither, MythX) miss: unauthenticated treasury broadcast, replay / missing nonce, web-layer idempotency, SSRF webhooks, fail-open auth, unlimited approvals. Zero-dependency Node CLI with a CI gate + GitHub Action and text/HTML/JSON reports, plus a free web grader. 15 rules drawn from the "Five Attacks on x402" taxonomy. `npx x402guard .` ([npm](https://www.npmjs.com/package/x402guard)) ([GitHub](https://github.com/dario933/x402guard)) ([grader](https://x402guard.io))
