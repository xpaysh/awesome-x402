...
- [Hermes Plant](https://hermesplant.com/agent-services) - Deterministic finance & quant APIs for AI agents: DCF / IRR / XIRR / NPV (CashflowLens), LP/GP distribution waterfalls (WaterfallLens), Black-Scholes pricing & Greeks (OptionLens), bond yield / duration / amortization (BondLens), portfolio risk (PortfolioGuard), wallet AML screening (WalletGuard), email validation (EmailGuard), MCP-server risk (mcp-risk), and a premium deal-underwriting composite (DealAnalyzer). Pure math, evidence-backed findings, no data feed — $0.02–$2.00 USDC per call on Base via the Coinbase CDP facilitator, no API keys or signup. (Models that "just do the IRR themselves" get it wrong: see the [LLM finance-math benchmark](https://hermesplant.com/benchmarks/llm-finance-math).) ([MCP](https://hermesplant.com/mcp)) ([OpenAPI](https://hermesplant.com/openapi.json)) ([x402 discovery](https://hermesplant.com/.well-known/x402)) ([llms.txt](https://hermesplant.com/llms.txt))
