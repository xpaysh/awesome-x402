...
- [SYNTHORA Contract Guard](https://pay.hergertsynthora.com/service) — On-chain contract safety check for AI agents: honeypot, rug-pull, hidden-mint and owner-privilege detection on Base. Deterministic engine — every response carries a verifiable **Ed25519-signed verdict** with full data provenance (on-chain reads + sha256). Pre-trade safety for agents before they swap or interact with a contract. $0.005 USDC on Base via CDP Facilitator, no accounts, no signup. ([OpenAPI](https://pay.hergertsynthora.com/openapi.json)) ([Discovery](https://pay.hergertsynthora.com/.well-known/x402))
- [SYNTHORA x402 Registry Watch](https://regwatch.hergertsynthora.com/service) — Signed Ed25519 delta feed of the live x402/M2M market: endpoint changes, payTo addresses, new listings and price movements. Pay-per-event for router/discovery agents — every response is **cryptographically signed** with full provenance. First 5 calls free per Base wallet. $0.02 USDC on Base via CDP Facilitator, no accounts, no signup. ([OpenAPI](https://regwatch.hergertsynthora.com/openapi.json)) ([Discovery](https://regwatch.hergertsynthora.com/.well-known/x402))
