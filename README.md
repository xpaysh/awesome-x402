...
- [Magpie](https://x402.magpie.capital) - The first **lending protocol** an AI agent can drive end-to-end over the x402 rail (Solana, native SOL): pay-per-call HTTP → get back an unsigned Solana transaction → sign it yourself. Zero custody — the service holds no keys, so even a full compromise can't move your funds. Borrow SOL against memecoin collateral, arm in-vault take-profit / stop-loss on your own loan, and repay — every tx agent-signed. TypeScript SDK + a 26-tool MCP server. ([SDK](https://www.npmjs.com/package/@magpieloans/magpie-agent)) ([MCP](https://www.npmjs.com/package/@magpieloans/magpie-mcp)) ([Docs](https://www.magpie.capital/x402))
