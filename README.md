...
- [Touchstone](https://touchstone.locomot.io) — Verify-before-you-pay truth + a deterministic workbench for agents. Every answer is signed by the key that takes payment and reproduces a *named* authority byte-for-byte, and every route has a free `/example` so an agent can judge worth before it pays. 14 endpoints in three wings — **facts**: arcsecond ephemeris (JPL DE421), the WMM2025 magnetic field, IANA-tz time/DST math, CMU-dict prosody; a drand-anchored **trust layer**: provably-fair randomness, beacon timestamps, timelock sealing, sealed-bid auctions; and a writing/coding **workbench**: citation formatting (APA/MLA/Chicago/BibTeX), rule-based proofreading, readability metrics, RFC-6902 JSON-Patch diffs, syntax validation, NIST SP 811 unit conversion, ECDSA signature recovery. $0.001–$0.01 USDC on Base (CDP facilitator), no keys, no signup. ([Discovery](https://touchstone.locomot.io/.well-known/x402)) ([OpenAPI](https://touchstone.locomot.io/openapi.json)) ([llms.txt](https://touchstone.locomot.io/llms.txt))
