...
- [Carbon & Cashmere](https://www.carbon-cashmere.de) — Crypto trading intelligence API with **166 operational x402 endpoints** (+ 11 sponsor tier) across **7 networks** (Base, Polygon, Arbitrum, World Chain, Solana, Algorand, Stellar). Powered by own blockchain nodes (Bitcoin Core pruned, Kaspa Rusty, Bittensor Subtensor, Ethereum Nimbus beacon) + proprietary ML ensemble (XGBoost + LightGBM + LSTM + ARF + Meta-learner). Coverage: signals/decisions, BTC/Kaspa/Subtensor derivatives, macro/funding/liquidations, RWA Trust Score, Tokenized-Equity Cross-Issuer Index. $0.005 commodity → $0.25 premium signals. x402 v2 with Bazaar enrichment, offer-receipt EIP-712, gas-sponsoring. MCP server `@carbon-cashmere/x402-mcp` on Anthropic Registry + Smithery + Glama + npm. ([Manifest](https://api.carbon-cashmere.de/.well-known/x402)) | ([Sponsor manifest](https://api.carbon-cashmere.de/.well-known/x402-sponsor)) | ([Docs](https://api.carbon-cashmere.de/docs)) | ([npm](https://www.npmjs.com/package/@carbon-cashmere/x402-mcp))
