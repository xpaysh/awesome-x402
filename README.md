...
- [PayAPI Market](https://payapi.market) - Marketplace for x402 APIs where the settlement-verified badge means the listing was paid for from the operator's own wallet and returned real product, each badge backed by an on-chain transaction on Base paid to the provider's declared wallet, not scored by heuristics. 28 live APIs from 15 independent providers, 9 currently carrying the badge. Coverage spans compliance and sanctions screening, company and business registries, government open data, and financial and agent tooling. $0.001-$0.50 USDC per request. MCP server for agent discovery. ([GitHub](https://github.com/chetparker/x402-marketplace))
