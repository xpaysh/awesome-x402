...
- [RugScore — Solana Token Risk API](https://solrisk.rugscore.workers.dev) - Pay-per-call rug / honeypot / dump risk score (0–100) for any Solana SPL token: mint & freeze authority, top-holder concentration, and liquidity/volume signals → JSON. $0.01 USDC on **Solana mainnet** via x402, settled directly on-chain (no facilitator, no account, no API key). Free preview score at `/demo?mint=<MINT>`, no payment. ([Discovery](https://solrisk.rugscore.workers.dev/.well-known/x402)) ([OpenAPI](https://solrisk.rugscore.workers.dev/openapi.json)) ([llms.txt](https://solrisk.rugscore.workers.dev/llms.txt))
